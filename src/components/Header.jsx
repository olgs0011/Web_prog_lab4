...
export const RightHeader = ({
  ...
  // Функция для обновления стилей кнопки закрытия
  const updateCloseMenuStyle = () => {
    const width = window.innerWidth;
    
    if (width <= 768) {
      // Для мобильных устройств меняем позиционирование кнопки закрытия
      setCloseMenuStyle({
        position: "fixed",
        top: "20px",
        right: "20px",
  ...
  useEffect(() => {
    updateCloseMenuStyle();
    
    // Добавляем слушатель изменения размера окна
    window.addEventListener("resize", updateCloseMenuStyle);
    
    return () => {
      window.removeEventListener("resize", updateCloseMenuStyle);
    };
  }, []);
...
const Header = () => {
  ...
  useEffect(() => {
    const updateBurgerState = () => {
      ...
      // Для экранов 1440px и выше показываем меню, скрываем бургер
      if (width >= 1440) {
      ...
      // Для экранов до 1440px показываем бургер
      if (width <= 1439 || window.isMobileBrowser()) {
        overflow();
